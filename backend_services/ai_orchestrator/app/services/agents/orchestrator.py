...
class FinancialOrchestrator:
    ...
    def _needs_deep_analysis(self, data: AnalysisPayload) -> bool:
        ...
        """
        # 1. Did the ML Anomaly detector find any severe outliers?
        if len(data.anomalies) > 0:
            return True
            
        # 2. Are trends severely negative? (e.g., latest growth < -10%)
        if data.trends and data.trends.get("latest_growth", 0) < -10.0:
            return True
            
        # 3. Are margins critically low? (< 5%)
        latest_margin = data.kpis.get("avg_margin", 0)
