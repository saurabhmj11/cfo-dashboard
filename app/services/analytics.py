...
from app.services.ml_anomaly import ml_anomaly_detector
...
class AnalyticsEngine:
    ...
    3. Output: Structred Metrics (FinancialAnalysisResult).
    """
    ...
    def compute_metrics(self, transactions: List[Transaction]) -> Dict[str, Any]:
        ...
        # Convert ORM objects to DataFrame for vectorised math
        data = [
        ...
        # 6. Calculate Confidence & Trust (Moved up to affect Legacy)
        ...
        # 6. Confidence already calculated above
    ...
    def _detect_statistical_anomalies(self, monthly_df: pd.DataFrame) -> List[Anomaly]:
        """
        Deprecated: Used Z-Score to find statistical outliers.
        Now relying on the MLAnomalyDetector (Isolation Forest).
        """
        pass

    def _calculate_data_confidence(self, monthly_agg: pd.DataFrame, raw_df: pd.DataFrame) -> Dict[str, Any]:
        """
        Calculates a Trust Score (0.0 to 1.0) based on:
        1. Data Density (Are there gaps?)
        2. Recency (Is the data old?)
        3. Variance (Is it too volatile?)
        """
        score = 1.0
        issues = []
        ...
        # 1. Check for Sparse Data (Density)
        # If we have less than 5 transactions per month on average, that's suspicious
        ...
        # 2. Check for Missing Months (Continuity)
        # Check if index (PeriodIndex) has gaps
        ...
        # 3. Check for Extreme Volatility (Variance)
        # If revenue jumps > 200% MoM, it reduces confidence
